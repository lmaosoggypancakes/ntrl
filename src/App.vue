...
const changeData = (newParkSearchData: Park) => {
  ...
  shownParks.value = allParks.value.filter(park => {
    const containsName = park.title.toLowerCase().includes(newParkSearchData.title.toLowerCase()) // normalize everything to lowercase to search for parks easier (no need to capitalize, more convenient user experience)
    ...
    const containsActivities = newParkSearchData.activities.every(activity => park.activities.includes(activity)) 
    const containsUtilities = newParkSearchData.utilities.every(utility => park.utilities.includes(utility))
    const containsTransportation = newParkSearchData.transportation.every(transportation => park.transportation.includes(transportation))
    ...
    const isWithinFilters = containsName && containsActivities && containsTransportation && containsUtilities
    if (newParkSearchData.dogabble) {
      return newParkSearchData.region.length == 0 ? isWithinFilters && park.dogabble : isWithinFilters && park.dogabble && park.region == newParkSearchData.region
    } else {
      return newParkSearchData.region.length == 0 ? isWithinFilters  : isWithinFilters && park.region == newParkSearchData.region
    }
  })
}
const saveData = () => {
...
onBeforeMount(async () => {
  ...
  allParks.value.forEach((park: Park) => {
    ...
    allActivities.value = Array.from(new Set(allActivities.value.concat(park.activities)))
    allUtilities.value = Array.from(new Set(allUtilities.value.concat(park.utilities)))
    allTransportations.value = Array.from(new Set(allTransportations.value.concat(park.transportation)))
  })
})

const openGoogleMapsPark = (park: Park) => {
...
<template>
  ...
  -->
  <div class="bg-green-200 w-screen h-screen p-2 flex flex-col flex-shrink-0 font-black overflow-x-hidden">
    <p class="text-3xl p-4 flex justify-center items-center space-x-2">
      <span>Find parks in your area!</span>
      <a href="https://github.com/lmaosoggypancakes/ntrl/tree/app" target="_blank">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 hover:fill-blue-700 hover:cursor-pointer" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
        </svg>
      </a>
    </p>

    <div class="border-4 rounded-md border-slate-800 bg-white grid grid-cols-2 gap-2 h-full w-full overflow-x-hidden">
      <div class="my-4 border-r-2 border-r-slate-900 grid grid-rows-12 min-h-0">
        <div class="mx-2 pb-8 border-b-2 border-slate-200 row-span-1">
          <SearchMenu @searchChange="changeData" :activities="allActivities" :utilities="allUtilities" :transportation="allTransportations"/>
        </div>
        <div class="w-full flex justify-center mt-2">
          <button class="flex justify-center items-center w-1/2 border-2 border-black rounded-lg hover:bg-gray-700 hover:text-white active:text-green-600 active:bg-white transition-colors" type="button" @click="saveData">
            <span>Save Data</span>
        ...
        <div class="overflow-y-auto row-span-11 ">
          <ul class="px-4">
            <li v-for="park in shownParks" class="border-2 border-black bg-orange-300 rounded-md w-full p-2  mt-2 hover:bg-orange-400" @mouseover="hoveredParkId = park.id" @click="openGoogleMapsPark(park)">
      ...
      <div class="overflow-hidden">
          <ol-map :loadTilesWhileAnimating="true" :loadTilesWhileInteracting="true" style="height:100%">
              <ol-view ref="view" :center="mapCenter" :rotation="mapRotation" :zoom="mapZoom" 
              :projection="mapProjection" />
              <ol-tile-layer>
                  <ol-source-osm />
              </ol-tile-layer>
              <ol-overlay v-for="park of shownParks" :position="[park.y, park.x]" :content="park.title" v-tippy> <!-- the v-tippy and content directive are used to declaratively and dynamically provide visual tooltips. -->
                <template v-slot="slotProps">
                    <!-- show a green icon if its corresponding park is hovered over, else just the black/white one -->
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" v-if="park.id == hoveredParkId">
                      <path class="fill-green-500" stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>

                    <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-67" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" v-else>
