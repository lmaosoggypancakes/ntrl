...
const regions = ref<string[]>(["Northern", "Southern", "Eastern"]);
...
const changeFilters = (
  value: string,
  t: "activities" | "utilities" | "transportation"
) => {
...
<template>
  <form class="p-2 flex flex-col space-y-8">
    <div class="flex flex-row justify-center">
      <input
        type="text"
        content="Search for park names!"
        v-tippy
        class="w-full mx-8 rounded-md bg-green-200 border-2 border-slate-700 px-2 text-gray-900 placeholder:text-slate-800"
      ...
      <button
        @click="show = !show"
        type="button"
        class="p-2 border-2 border-slate-900 rounded-md"
    ...
    <transition :name="slide" class="overflow-hidden">
      <!-- transition element wraps the content such that it's state transitions (open/show) can
            be animated by sliding up/down. might not work in legacy browsers or older PC's-->
      <div
        :class="{ hidden: show }"
        class="border-2 border-slate-800 rounded-md p-2 flex flex-col space-y-4"
      >
          ...
          <div>
            ...
            <div v-for="activity in activities">
              ...
              <input
                ...
                @click="changeFilters(activity, 'activities')"
              />
            </div>
          </div>
          <div>
            ...
            <div v-for="utility in utilities">
              ...
              <input
                ...
                :value="utility"
                class="float-right mr-8"
                @click="changeFilters(utility, 'utilities')"
              />
            </div>
          </div>
          <div>
            ...
            <div v-for="t in transportation">
              ...
              <input
                ...
                :value="t"
                class="float-right mr-8"
                @click="changeFilters(t, 'transportation')"
              />
            </div>
          </div>
        </div>
